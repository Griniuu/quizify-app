    ...
    <div v-else-if="showResults && results" class="results-container">
      <div class="card shadow-lg">
        <div class="card-body p-5 text-center">
          ...
          <!-- Tytuł wyniku -->
          <h2 class="mb-3">
            <span v-if="results.passed" class="text-success">Gratulacje! 🎉</span>
            <span v-else class="text-warning">Spróbuj ponownie</span>
          </h2>

          <!-- Wynik procentowy -->
          <div class="score-display mb-4">
            <div class="display-3 fw-bold" :class="results.passed ? 'text-success' : 'text-warning'">
              {{ results.percentage }}%
            </div>
          ...
          <div v-if="results.wrongAnswers > 0 && results.incorrectDetails" class="mt-4">
            <button 
            ...
            >
              ...
              {{ showIncorrectAnswers ? 'Ukryj' : 'Pokaż' }} błędne odpowiedzi ({{ results.wrongAnswers }})
            </button>

            <div v-if="showIncorrectAnswers" class="incorrect-answers-list mt-3">
              <div 
                  ...
                  <div class="row g-2">
                    <div class="col-md-6">
                      <div class="alert alert-danger mb-0 py-2">
                        <small class="d-block text-muted mb-1">Twoja odpowiedź:</small>
                        <strong>{{ item.userAnswer }}</strong>
                      </div>
                    </div>
                    <div class="col-md-6">
                      <div class="alert alert-success mb-0 py-2">
                        <small class="d-block text-muted mb-1">Poprawna odpowiedź:</small>
...
import { fetchQuiz, submitQuizAnswers } from '../services/mockApi'
...
async function finishQuiz() {
  ...
  try {
    ...
    const response = await submitQuizAnswers(quiz.value.id, userAnswers.value)
    ...
    error.value = 'Nie udało się wysłać odpowiedzi. Spróbuj ponownie.'
